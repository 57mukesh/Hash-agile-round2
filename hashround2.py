...
class ElasticsearchOperations:
    def __init__(self, host='localhost', port=8989, username='elastic', password='tyMOLKaGVsTlaaftZ=V_'):
        ...
        self.file_path = r"C:\\Users\\mukes\\Downloads\\Employee Sample Data 1.csv"
...
def main():
    ...
    es_ops = ElasticsearchOperations(
        port=8989,
        username='elastic',
        password='tyMOLKaGVsTlaaftZ=V_')
    
    
    v_nameCollection = 'Hash_Mukesh'
    v_phoneCollection = 'Hash_3506'
    
    print("\n=== Starting Elasticsearch Operations ===\n")
    
    # Step a
    print("\nA) Setting v_nameCollection = 'Hash_Mukesh'...")
    print(f"v_nameCollection = {v_nameCollection}")
    
    # Step b
    print("\nB) Setting v_phoneCollection = 'Hash_3506'...")
    print(f"v_phoneCollection = {v_phoneCollection}")
    
    # Step c
    print("\nC) Creating collection 'Hash_Mukesh'...")
    es_ops.createCollection(v_nameCollection)
    
    # Step d
    print("\nD) Creating collection 'Hash_3506'...")
    es_ops.createCollection(v_phoneCollection)
    
    # Step e
    print("\nE) Getting Initial Employee count for 'Hash_Mukesh'...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step f
    print("\nF) Indexing data in 'Hash_Mukesh' on column 'Department'...")
    es_ops.indexData(v_nameCollection, 'Department')
    
    # Step g
    print("\nG) Indexing data in 'Hash_3506' on column 'Gender'...")
    es_ops.indexData(v_phoneCollection, 'Gender')
    
    # Step h
    print("\nH) Getting updated employee count for 'Hash_Mukesh'...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step i
    print("\nI) Deleting employee with ID 'E02003' from 'Hash_Mukesh'...")
    es_ops.delEmpById(v_nameCollection, 'E02003')
    
    # Step j
    print("\nJ) Getting employee count for 'Hash_Mukesh' after deletion...")
    es_ops.getEmpCount(v_nameCollection)
    
    # Step k
    print("\nK) Searching 'Hash_Mukesh' for employees in Department 'IT'...")
    es_ops.searchByColumn(v_nameCollection, 'Department', 'IT')
    
    # Step l
    print("\nL) Searching 'Hash_Mukesh' for employees with Gender 'Male'...")
    es_ops.searchByColumn(v_nameCollection, 'Gender', 'Male')
    
    # Step m
    print("\nM) Searching 'Hash_3506' for employees in Department 'IT'...")
    es_ops.searchByColumn(v_phoneCollection, 'Department', 'IT')
    
    # Step n
    print("\nN) Getting department facets for 'Hash_Mukesh'...")
    es_ops.getDepFacet(v_nameCollection)
    
    # Step o
    print("\nO) Getting department facets for 'Hash_3506'...")
